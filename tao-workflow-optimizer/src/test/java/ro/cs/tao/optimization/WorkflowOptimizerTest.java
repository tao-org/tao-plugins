...
public class WorkflowOptimizerTest {
    private static void printWorkflow(WorkflowDescriptor workflow) {
        for (WorkflowNodeDescriptor wNode : workflow.getNodes()) {
            System.out.print("(" + wNode.getComponentId() + ") - [");

            for (ComponentLink link : wNode.getIncomingLinks()) {
                System.out.print("(" + link.getOutput().getId() + "), ");
            }
    ...
    private static WorkflowDescriptor createAndAddNodes() throws Exception {
        WorkflowDescriptor workflow = new WorkflowDescriptor();

        BufferedReader reader = new BufferedReader(
                new InputStreamReader(
                        WorkflowOptimizerTest.class.getResourceAsStream("test.json")));
        ...
        if (processingComponents.length != 5) {
            return null;
        }
    ...
    public static void main(String[] args) throws Exception {
        ...
        WorkflowDescriptor workflow = createAndAddNodes();

        printWorkflow(workflow);

        WorkflowDescriptor optimized = WorkflowOptimizer.getOptimizedWorkflow(workflow);

        printWorkflow(optimized);
    }
}
