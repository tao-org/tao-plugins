 ...
 */
package ro.cs.tao.datasource.remote.eocat.download;
 ...
 */
public class EOCATDownloadStrategy extends DownloadStrategy<Header> {
    private static final String ZIP_EXTENSION = ".zip";
    private static final long DOWNLOAD_TIMEOUT = 30000; // 30s
    private Timer timeoutTimer;

    public EOCATDownloadStrategy(EOCATDataSource dataSource, String targetFolder, Properties properties) {
        super(dataSource, targetFolder, properties);
    }

    protected EOCATDownloadStrategy(EOCATDownloadStrategy other) {
        super(other);
    }


    @Override
    public EOCATDownloadStrategy clone() {
        return new EOCATDownloadStrategy(this);
    }
    ...
    protected HttpURLConnection getConnectionForProduct(EOProduct product) {
        ...
        try {
            ...
            authProperties.setProperty(EOCATAuthentication.DOWNLOAD_URL_PROPERTY_NAME, productDownloadUrl);
                    ...
                    task = new TimerTask() {
                        ...
                        public void run() {
                            logger.warning(String.format("Remote host did not send anything for %d seconds, cancelling download",
                                    connTimeout / 1000));
                            EOCATDownloadStrategy.this.cancel();
                        }
                    };
