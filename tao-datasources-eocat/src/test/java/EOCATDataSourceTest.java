...
import ro.cs.tao.datasource.remote.eocat.download.EOCATDownloadStrategy;
 ...
 * @author Cosmin Cara
 */
public class EOCATDataSourceTest {
...
//        groupCollectionsByQueryAuthentication(dataSource);
    ...
    private static DataSource initAndGetDatasource() {
        ...
        DataSource dataSource = serviceRegistry.getService(EOCATDataSource.class);
    ...
    private static String getPlatformName(DataSource dataSource, int platformIndex) {
        ...
        if (platforms.length < 1) {
            throw new IllegalStateException("no platforms for EOCAT");
        }
    ...
    private static void testDownload(DataSource dataSource, String platform, List<EOProduct> results) {
        try {
            final EOCATDownloadStrategy strategy = (EOCATDownloadStrategy) dataSource.getProductFetchStrategy(platform);
            if (!results.isEmpty()) {
    ...
    private static void kompsat2_Test(DataSource dataSource) {
        String kompsat2Platform = getPlatformName(dataSource, 19);
    ...
    private static void envisat_Test(DataSource dataSource) {
        String envisatPlatform = getPlatformName(dataSource, 11);
