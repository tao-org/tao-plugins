 ...
 */
public class SmapDownloadStrategy extends DownloadStrategy<String> {
    ...
    public SmapDownloadStrategy(ASFDataSource dataSource, String targetFolder, Properties properties) {
        super(dataSource, targetFolder, properties);
    }

    protected SmapDownloadStrategy(SmapDownloadStrategy other) {
        super(other);
    }


    @Override
    public SmapDownloadStrategy clone() {
        return new SmapDownloadStrategy(this);
    }
    ...
    protected HttpURLConnection getConnectionForProduct(EOProduct product) {
        ...
        try {
            HttpURLConnection connection = NetUtils.openConnection(productDownloadUrl, this.dataSource.authenticate());
            ...
            do {
                ...
                switch (responseStatus) {
                    case HttpStatus.SC_OK:
                        connection = NetUtils.openConnection(connection.getURL().toString(), this.dataSource.authenticate());
                        connection.setInstanceFollowRedirects(true);
                        connection.connect();
                        return connection;
    ...
    protected Path downloadProduct(EOProduct product, HttpURLConnection connection) throws IOException {
        ...
        try {
            ...
            String archiveName;
            if (product.getAttributeValue("fileName") != null) {
                archiveName = product.getAttributeValue("fileName");
            } else {
                archiveName = (product.getId() != null ? product.getId() : product.getName()) + extension;
            }
                    ...
                    task = new TimerTask() {
                        ...
                        public void run() {
                            logger.warning(String.format("Remote host did not send anything for %d seconds, cancelling download",
                                    connTimeout / 1000));
                            SmapDownloadStrategy.this.cancel();
                        }
                    };
