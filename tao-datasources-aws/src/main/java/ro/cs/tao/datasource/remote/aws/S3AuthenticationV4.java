...
class S3AuthenticationV4 {
    ...
    static final String REQUEST_PAYER_HEADER_NAME = "x-amz-request-payer";
    /**
    ...
    private final List<NameValuePair> customParameters;

    private LocalDateTime creationDate;
    private LocalDateTime expirationDate = null;
    private List<NameValuePair> awsHeaders;
     ...
     */
    S3AuthenticationV4(String httpVerb, String region, String accessKeyId, String secretAccessKey, List<NameValuePair> customParameters) {
    ...
    private static List<NameValuePair> getRequestParametersList(String query) {
        ...
        if (query != null && !query.isEmpty()) {
            ...
            for (String requestParameter : requestParametersArray) {
                ...
                requestParameters.add(new BasicNameValuePair(requestParameterName, requestParameterValue));
            }
    ...
    private List<NameValuePair> buildAwsHeaders(URL url) {
        List<NameValuePair> headers = new ArrayList<>();
        headers.add(new BasicNameValuePair(AWS_HOST_HEADER_NAME, url.getHost()));
        headers.add(new BasicNameValuePair(AWS_CONTENT_SHA256_HEADER_NAME, AWS_CONTENT_SHA256_HEADER_VALUE));
        headers.add(new BasicNameValuePair(AWS_DATE_HEADER_NAME, getISODateTime()));
        for (NameValuePair customParameter : this.customParameters) {
            if (!customParameter.getName().contentEquals(AWS_CONTENT_SHA256_HEADER_NAME) && !customParameter.getName().contentEquals(AWS_DATE_HEADER_NAME)) {
                headers.add(new BasicNameValuePair(customParameter.getName(), customParameter.getValue()));
            }
        }
        return headers;
    }

    private String buildCanonicalQueryString(List<NameValuePair> requestParameters) {
    ...
    private String buildCanonicalHeaders() {
        ...
        for (NameValuePair awsHeader : this.awsHeaders) {
            canonicalHeaders.append(lowercase(awsHeader.getName()));
    ...
    private String buildSignedHeaders() {
        ...
        for (NameValuePair awsHeader : this.awsHeaders) {
            ...
            signedHeaders.append(lowercase(awsHeader.getName()));
        }
    ...
    private boolean isPublic() {
        return this.accessKeyId == null || this.accessKeyId.isEmpty() || this.secretAccessKey == null || this.secretAccessKey.isEmpty();
    }
     ...
     */
    List<NameValuePair> getAwsHeaders(URL url) {
        if (!isPublic() && (this.awsHeaders == null || this.lastAuthorizedURL == null || !url.toString().contentEquals(this.lastAuthorizedURL.toString()))) {
    ...
    private static final class PropertyCodePointSorter implements Comparator<NameValuePair> {
